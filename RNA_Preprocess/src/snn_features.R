...
norm.data <- ScaleData(object = norm.data, features = hvg, vars.to.regress = c("nCount_RNA", "percent.mito"))
norm.data <- RunPCA(object = norm.data, features = hvg, verbose = FALSE)

# norm.data <- ScaleData(object = norm.data)
# norm.data <- RunPCA(object = norm.data)

##run snn clustering
norm.data <- FindNeighbors(object = norm.data, dims = 1:param$npc, k.param = param$k)
